...
from math import floor,pi,atan2,sqrt,sin,cos,exp,log,atan,log10
import numpy as np
from invjday import invjday
from finddays import finddays
...
def JB2008(MJD,r_Sun,r_SAT):
    ...
    i = np.nonzero(year==DTCdata[0,:])
    i=i[0]



    j=np.nonzero(floor(doy)==DTCdata[1,:])
    j=j[0]
    ...
    if SAT[2] > 105:
        ...
        CAPPHI = TRASH-1*floor(TRASH/1)

        DLRSL = 0.02 * (SAT[2] - 90) * exp(-0.045 * (SAT[2] - 90))\
                * sign_(1,SAT[1]) * sin(TWOPI * CAPPHI+ 1.72)\
          * sin(SAT[1])**2
        ...
        for I in range(1,7) :
            ...
            AN = exp(ALN[I-1])
            SUMN = SUMN + AN
            SUMNM = SUMNM + AN*AMW[I-1]
            AL10N[I-1] = ALN[I-1]/AL10
        ...
        if ((ZHT>=1000) and (ZHT<1500)):
            ZETA   = (ZHT - 1000) * 0.002
            ZETA2  =  ZETA * ZETA
            ZETA3  =  ZETA * ZETA2
            F15C   = CHT[0] + CHT[1]*F10B + CHT[2]*1500 + CHT[3]*F10B*1500
...
def XLOCAL(Z,TC):
    ...
    if (DZ > 0):
        ...
        XLOCAL2 = TC[0] + TC[2] * atan(TC[3]*DZ*(1 + 4.5e-6*DZ**2.5))

    else:

        XLOCAL2 = ((-9.8204695e-6 * DZ - 7.3039742e-4) * DZ**2 + 1) * DZ * TC[1] + TC[0]
...
def DTSUB(F10,XLST,XLAT,ZHT):
    ...
    if (ZHT>=120 and ZHT<=200):
        ...
        DTC200 = C[16]   + C[17]*tx*ycs      + C[18]*tx**2*ycs\
                 + C[19]*tx**3*ycs   + C[20]*F*ycs       + C[21]*tx*F*ycs\
                 + C[22]*tx**2*F*ycs
        sum = C[0] + B[1]*F + C[2]*tx*F     + C[3]*tx**2*F\
    ...
    if (ZHT>200 and ZHT<=240):
        ...
        sum = C[0]*H + B[1]*F*H + C[2]*tx*F*H     + C[3]*tx**2*F*H\
            ...
            + C[16]             + C[17]*tx*ycs      + C[18]*tx**2*ycs\
            + C[19]*tx**3*ycs   + C[20]*F*ycs       + C[21]*tx*F*ycs\
            + C[22]*tx**2*F*ycs
        DTC = sum


    if (ZHT>240 and ZHT<=300):
        ...
        BB = C[0] + B[1]*F  + C[2]*tx*F       + C[3]*tx**2*F\
           + C[4]*tx**3*F    + C[5]*tx**4*F    + C[6]*tx**5*F\
           + C[7]*tx*ycs     + C[8]*tx**2*ycs  + C[9]*tx**3*ycs\
           + C[10]*tx**4*ycs + C[11]*tx**5*ycs + C[13]*ycs\
           + C[14]*F*ycs     + C[15]*tx*F*ycs  + C[16]*tx**2*F*ycs

        H   = 300/100

        sum = B[0]    + B[1]*F  + B[2]*tx*F         + B[3]*tx**2*F\
            + B[4]*tx**3*F      + B[5]*tx**4*F      + B[6]*tx**5*F\
            + B[7]*tx*ycs       + B[8]*tx**2*ycs    + B[9]*tx**3*ycs\
            + B[10]*tx**4*ycs   + B[11]*tx**5*ycs   + B[12]*H*ycs\
            + B[13]*tx*H*ycs    + B[14]*tx**2*H*ycs + B[16]*tx**3*H*ycs\
            + B[16]*tx**4*H*ycs + B[17]*tx**5*H*ycs + B[18]*ycs
    ...
    if (ZHT>300 and ZHT<=600):
        ...
        sum = B[0]    + B[1]*F  + B[2]*tx*F         + B[3]*tx**2*F\
            ...
            + B[7]*tx*ycs       + B[8]*tx**2*ycs    + B[9]*tx**3*ycs\
            + B[10]*tx**4*ycs   + B[11]*tx**5*ycs   + B[12]*H*ycs\
            + B[13]*tx*H*ycs    + B[14]*tx**2*H*ycs + B[15]*tx**3*H*ycs\
            + B[16]*tx**4*H*ycs + B[17]*tx**5*H*ycs + B[18]*ycs
        DTC = sum


    if (ZHT>600 and ZHT<=800):

        ZP = (ZHT - 600)/100
        HP = 600./100.

        AA  = B[0]    + B[1]*F  + B[2]*tx*F         + B[3]*tx**2*F\
            + B[4]*tx**3*F      + B[5]*tx**4*F      + B[6]*tx**5*F\
            + B[7]*tx*ycs       + B[8]*tx**2*ycs    + B[9]*tx**3*ycs\
            + B[10]*tx**4*ycs   + B[11]*tx**5*ycs   + B[12]*HP*ycs\
            + B[13]*tx*HP*ycs   + B[14]*tx**2*HP*ycs+ B[15]*tx**3*HP*ycs\
            + B[16]*tx**4*HP*ycs + B[17]*tx**5*HP*ycs + B[18]*ycs
...
def SEMIAN08(DAY,HT,F10B,S10B,XM10B):
    # %
    ...
    FZZ = FZM[0] + FZM[1]*FSMB  + FZM[2]*FSMB*HTZ \
        + FZM[3]*FSMB*HTZ**2    + FZM[4]*FSMB**2*HTZ
    ...
    GTZ = GTM[0] + GTM[1]*SIN1P + GTM[2]*COS1P \
